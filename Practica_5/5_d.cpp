...
private:
        ...
        else
        {
            if(c == EOF || c == ';')
            {
                ...
                if(!idMode){
                    ...
                    cout<<"\nResultado: "<<resolvePost(getPost())<<endl;
                }
    ...
    Token getNextToken() {
        ...
        if (isalpha(c)) {
            ...
            if(token.value == "int")
                {token.type = TokenType::INT;idMode = true;return token;}
            auto pos = findIndex(ids, token.value);
            if(pos != -1)
                {token.type = TokenType::NUM;token.value = values[pos];return token;}
    ...
    void Exp() {
        if (currentToken.type == TokenType::INT) {
            Exp1();
        } else if (currentToken.type == TokenType::NUM || currentToken.type == TokenType::PARENTESIS_IZQ) {
            Exp2();
        } else {
    ...
    void Exp1() {
        if (currentToken.type == TokenType::INT) {
            match(TokenType::INT);
            ids.push_back(currentToken.value);
            match(TokenType::ID);
            match(TokenType::ASIGN);
            Term();
        }
        else {
            cerr << "Error de sintaxis en Exp1." << endl;
            error();
        }
    }
    void Exp2() {
        if (currentToken.type == TokenType::NUM) {
            ...
            cerr << "Error de sintaxis en Exp2." << endl;
            error();
        }
    }
    ...
    void Term() {
        if (currentToken.type == TokenType::NUM) {
            if(idMode) values.push_back(stoi(currentToken.value));
            match(TokenType::NUM);
...
public:
    ...
    void parse() {
        ...
        if (currentToken.type == TokenType::FIN) {
            ...
            cerr << "\nError: Caracteres adicionales después del final del analisis sintáctico." << endl;
            error();
        }
    }
    string getPost()
